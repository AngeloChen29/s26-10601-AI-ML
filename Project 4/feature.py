...
def extract_features(infile, outfile, glove):
    dataset = load_tsv_dataset(infile)
    ...
    with open(outfile, "w") as out:
        for (label, review) in dataset:
            ...
            out_line = f"{label:.6f}" + vector_str + "\n"
...
if __name__ == '__main__':
    ...
    extract_features(args.train_input, args.train_out, glove)
    extract_features(args.validation_input, args.validation_out, glove)
    extract_features(args.test_input, args.test_out, glove)
