...
class SelfAttention(nn.Module):
    ...
    def __init__(self, hidden_dim, key_dim, value_dim):
        ...
        self.query_transform = nn.Linear(self.hidden_dim, self.key_dim)
        self.key_transform = nn.Linear(self.hidden_dim, self.key_dim)
        self.value_transform = nn.Linear(self.hidden_dim, self.value_dim)
...
class RNN(nn.Module):
    ...
    def forward(self, sequence: Tensor) -> Tensor:
        ...
        output_states = torch.stack(output_states, 1)
...
class RNNLanguageModel(nn.Module):
    ...
    def forward(self, tokens: Tensor) -> Tensor:
        ...
        hidden_states, output_states = self.rnn(x)

        att = self.attention(output_states)
