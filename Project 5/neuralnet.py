...
What you need to do:
- Complete random_init                      done
- Implement SoftMaxCrossEntropy methods     done
- Implement Sigmoid methods                 done
- Implement Linear methods                  done
- Implement NN methods                      done
...
class SoftMaxCrossEntropy:
    ...
    def _cross_entropy(self, y: int, y_hat: np.ndarray) -> float:
        ...
        """
        # TODO: implement
        return -np.log(y_hat[y] + 0.00000000000001)
...
class Sigmoid:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        return dz * self.store * (1 - self.store)
...
class ReLU:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        dx = dz * (self.store > 0).astype(float)
        return dx
...
class Linear:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        g_alpha = np.outer(dz, x_copy)
        self.dw = g_alpha

        g_a = np.dot(np.transpose(self.w[:, 1:]), dz)
        return g_a
...
class NN:
    def __init__(self, input_size: int, hidden_size: int, hidden_size2: int, output_size: int,
                 weight_init_fn: INIT_FN_TYPE, learning_rate: float, activation_class: int):
        """
        ...
        self.linear1 = Linear(input_size, hidden_size, weight_init_fn, lr)
        
        self.linear2 = Linear(hidden_size, hidden_size2, weight_init_fn, lr)
        
        self.linear3 = Linear(hidden_size2, output_size, weight_init_fn, lr)

        if activation_class==1:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        # Output layer
        a3 = self.linear3.forward(z2)
        y_hat = self.smce._softmax(a3)

        # Compute cross-entropy loss for this example
        J = self.smce._cross_entropy(y, y_hat)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        :param y_hat: prediction with shape (num_classes,)
        """
        # TODO: call backward pass for each layers
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        num_points = X_tr.shape[0]

        for epoch in range(n_epochs):
            X_copy, y_copy = shuffle(X_tr, y_tr, epoch)

            for i in range(num_points):
                x_i = X_copy[i]
                y_i = y_copy[i]

                y_hat, J = self.forward(x_i, y_i)
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        err = np.mean(labels != y)

        return labels, err
        
def train_vary_hidden_units(X_tr, y_tr, X_val, y_val, hidden_units_list, 
                            epochs=100, lr=0.001):
    train_losses = []
    val_losses = []
    ...
    for n_hid in hidden_units_list:
        nn_model = NN(
            ...
            hidden_size=n_hid,
            hidden_size2=n_hid,
            output_size=len(np.unique(y_tr)),
        ...
        # train network
        nn_model.train(X_tr, y_tr, X_val, y_val, n_epochs=epochs)

        # compute final epoch losses
        train_loss = nn_model.compute_loss(X_tr, y_tr)
        val_loss = nn_model.compute_loss(X_val, y_val)
...
if __name__ == "__main__":
    ...
    nn_relu = NN(
        ...
        weight_init_fn=random_init,
        learning_rate=lr,
        activation_class=2  # if your NN class allows this
    )
    ...
    train_losses, val_losses = train_vary_hidden_units(
        X_tr, y_tr, X_test, y_test, hidden_units_list, epochs=100, lr=0.001
    )

    plt.figure(figsize=(10,6))
    plt.plot(hidden_units_list, train_losses, marker='o', label='Train Loss')
    plt.plot(hidden_units_list, val_losses, marker='s', label='Validation Loss')
    plt.xlabel('Number of Hidden Units')
    plt.ylabel('Average Cross-Entropy Loss')
    plt.title('Average Train and Validation Cross-Entropy Loss vs Hidden Units')
    plt.legend()
    plt.grid(True)
    plt.show()
